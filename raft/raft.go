...
func (rf *Raft) manageFollower() {
	...
	if time.Since(lastAccessed).Milliseconds() >= duration.Milliseconds() {
		...
		rf.votedFor = -1
		rf.persist()
		DPrintf("Election timeout : Server %d is now Candidate", rf.me)
		rf.mu.Unlock()
	}
}
...
func (rf *Raft) manageCandidate() {
	...
	for peer := range peers {
		...
		go func(peer int) {
			...
			defer rf.mu.Unlock()
			if !ok {
				DPrintf("Server %d not responding to request vote RPC", peer)
				finished++
				rf.cond.Broadcast()
				return
			}
			if reply.VoteGranted {
				DPrintf("Server %d granted vote to Server %d", peer, rf.me)
				finished++
				count++

			} else {
				DPrintf("Server %d denied vote to Server %d", peer, rf.me)
				finished++
				if args.Term < reply.Term {
					DPrintf("Server %d with higher term, So Server %d is now follower", peer, rf.me)
					rf.status = Follower
					rf.persist()
				}
			}
	...
	go rf.timer(timeOut)
	rf.mu.Lock()
	for count < majority && finished != total && time.Since(start).Milliseconds() < timeOut.Milliseconds() {
		rf.cond.Wait()
	}

	if time.Since(start).Milliseconds() >= timeOut.Milliseconds() && rf.status == Candidate {
		rf.currentTerm++
		rf.votedFor = -1
		DPrintf("Election Failed, Restarting election with new term")
		rf.persist()
		rf.mu.Unlock()
		return
	}
	...
	if rf.status == Candidate && count >= majority {
		DPrintf("Election Success Server %d is Leader", rf.me)
		rf.status = Leader
		for peer := range peers {
...
func (rf *Raft) manageLeader() {
	...
	for peer := range peers {
		...
		go func(peer int) {
			...
			if !ok {
				DPrintf("Server %d not respnding to append entries RPC", peer)
				return
			}

			rf.mu.Lock()
			if reply.Success {
				rf.nextIndex[peer] = rf.nextIndex[peer] + len(args.Entry)
				rf.matchIndex[peer] = prevLogIndex + len(args.Entry)
				DPrintf("Server %d Append Entries Success", peer)
			} else {
				...
				if reply.Xterm == -1 {
					...
					DPrintf("Server %d Log mismatch at log index %d", peer, rf.nextIndex[peer])
					rf.mu.Unlock()
					return
				}
				index := -1
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.applyMsg = applyCh
	DPrintf("Server %d initialised", rf.me)
...
func (rf *Raft) timer(dur time.Duration) {
	time.Sleep(dur)
	rf.mu.Lock()
	rf.cond.Broadcast()
	rf.mu.Unlock()
